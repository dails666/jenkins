...
from cmdata.select_data import ep_month_to_date_margin
from cmdata.api import get_enterprise_detail
import re


class TestMonthToDateMargin(unit.Unit):

    # 查询企业月至今毛利额
    def test_ep_month_to_date_margin(self):
        ...
        try:
            for cmid, ep_info in cmids.items():
                ...
                data = '%.2f' % (ep_month_to_date_margin.month_to_date_margin(cmid, source_id, item_cat1_ignores, days_delay))
                data_sql = re.sub(r"(\d)(?=(\d\d\d)+(?!\d))", r"\1,", data) + u'元'
                data_api = get_enterprise_detail.get_enterprise_detail(token)['margin']['month_to_date']
                if data_sql == data_api:
                    print(u'%s月至今毛利额数据正常！' % (cmid))
                else:
                    print(u'%s月至今毛利额数据异常：' % (cmid)),
